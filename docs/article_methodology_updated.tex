...
\For{$t = 1$ to $T_{\mathrm{PSO}}$}
    \For{each particle $p$ in the swarm}
        ...
        \EndIf
        \State Build the ANN and train it under the low-fidelity budget
...
\For{each shortlisted architecture}
    \For{each Stage 2 hyperparameter setting in the tuning grid}
        \State Retrain the ANN under the larger Stage 2 budget
        \State Evaluate it on the same held-out validation engines
        \State Rank the run with the complexity-aware selection score
    \EndFor
\EndFor
\State Select the best retuned candidate $\mathcal{M}^{\star}$
\State Refit the feature scaler on the full official training split
\State Rebuild all official training windows and retrain $\mathcal{M}^{\star}$ on the full official training split
...
\State Evaluate $\mathcal{M}^{\star}$ once on the official benchmark test split and report the final metrics
\end{algorithmic}
\end{algorithm}
